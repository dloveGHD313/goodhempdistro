...
export type PostAuthorTier = "vip" | "enterprise" | "pro" | "top" | "mid" | "starter" | "none";
...
export function getPostBadgeLabel(role: PostAuthorRole, tier: PostAuthorTier, isAdminPost: boolean): string | null {
  ...
  if (role === "vendor") {
    if (tier === "vip" || tier === "top" || tier === "enterprise") return "Enterprise Vendor";
    if (tier === "mid" || tier === "pro") return "Pro Vendor";
    return null;
  }
  if (role === "consumer") {
