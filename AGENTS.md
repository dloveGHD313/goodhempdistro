...
- Never edit generated files under `.next/**` (including `.next/types/validator.ts`). If you hit a "file is newer" save conflict: close the generated file without overwriting, delete `.next`, restart the TypeScript server in Cursor, and rerun `npm run dev` or `npm run build`.
