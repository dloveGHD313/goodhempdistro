...
export async function POST(
  ...
  const logStage = (stage: string, details?: Record<string, unknown>) => {
    console.log(`[admin/events/approve][${requestId}] ${stage}`, details || {});
  };
  try {
    logStage("start");
    ...
    if (eventError || !event) {
      if (eventError) {
        console.error(`[admin/events/approve][${requestId}] event_fetch_error`, {
    ...
    if (updateError && /column .* does not exist/i.test(updateError.message || "")) {
      console.warn(`[admin/events/approve][${requestId}] column_missing_retry`, {
    ...
    if (updateError) {
      console.error(`[admin/events/approve][${requestId}] update_error`, {
    ...
    console.error(`[admin/events/approve][${requestId}] unexpected_error`, error);
    return NextResponse.json(
