...
const resolveVendorPriceId = (planName: string | null) => {
  ...
  if (normalized.includes("starter") || normalized.includes("basic")) {
    return process.env.STRIPE_VENDOR_STARTER_PRICE_ID || null;
  }
  if (normalized.includes("pro")) {
    return process.env.STRIPE_VENDOR_PRO_PRICE_ID || null;
  }
  if (normalized.includes("enterprise") || normalized.includes("elite")) {
    return process.env.STRIPE_VENDOR_ENTERPRISE_PRICE_ID || null;
  }
  return null;
};
