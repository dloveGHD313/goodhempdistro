...
export default function VendorReferralPayoutsClient() {
  ...
  const fetchPayouts = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/admin/vendor-referrals/payouts", { cache: "no-store" });
  ...
  const handleApprove = async (id: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/vendor-referrals/payouts/${id}/approve`, { method: "POST" });
      const data = await res.json();
      if (!res.ok) {
          ...
          <table className="w-full text-left">
            <thead className="border-b border-[var(--border)]">
              <tr>
                ...
                <th className="pb-2 font-semibold text-muted">Vendor</th>
            ...
            <tbody>
              {payouts.map((p) => (
                <tr key={p.id} className="border-b border-[var(--border)]/60">
                  ...
                  <td className="py-2 font-mono text-sm">{p.referral_code ?? "—"}</td>
                  <td className="py-2 font-semibold">${(p.amount_cents / 100).toFixed(2)}</td>
                  <td className="py-2">
                    <span
                      className={`px-2 py-0.5 rounded text-xs ${
                        p.status === "paid" ? "bg-green-500/20 text-green-400" : p.status === "requested" ? "bg-amber-500/20 text-amber-400" : "bg-gray-500/20 text-gray-400"
                      }`}
                    >
                      {p.status}
                    </span>
